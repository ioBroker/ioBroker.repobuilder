...
const getNpmVersion = require('./npm').getNpmVersion;
...
async function getIoPack(source) {
    ...
    if (ioPack?.common) {
        ...
        if (source.licenseInformation && (source.licenseInformation.type === 'free' || source.licenseInformation.type === undefined)) {
...
async function readGithubStats(sources) {
    ...
    /* if (FAST_TEST) {
        return callback && callback(null, sources);
    }*/
    let adapter;
    do {
        let index;
        ...
        if (adapter) {
            let timeout = 0;
            DEBUG && console.log(`Get GitHub stats for ${adapter}`);
            try {
