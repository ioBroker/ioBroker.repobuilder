    ...
    for (const adapter in sources) {
        if (sources[adapter].weekDownloads === undefined && !adapter.startsWith('_')) {
            ...
            try {
                ...
                console.error(`Status code is not 200 (${error.status}) [${url}]: ${JSON.stringify(error.response?.data || error.message || error.code)}`);
            }
...
async function readUrlBinary(url) {
    ...
    try {
        ...
        console.error(`Status code is not 200 (${error.status}) [${url}]: ${JSON.stringify(error.response?.data || error.message || error.code)}`);
        throw new Error(JSON.stringify(error.response?.data || error.message || error.code));
    }
}
async function readNpmIoPack(name, version) {
    let data;
    const cachedFileName = (0, node_path_1.join)((0, node_os_1.tmpdir)(), `iobroker.${name}-${version}.tgz`);
    if ((0, node_fs_1.existsSync)(cachedFileName)) {
        data = (0, node_fs_1.readFileSync)(cachedFileName);
    }
    else {
        ...
        (0, node_fs_1.writeFileSync)(cachedFileName, data);
    }
